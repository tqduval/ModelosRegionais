...
for kmodel = 1:length(models(:,1))
  modelIndex = find(models(kmodel,:)>0);    %% Identify the columms from the data to be used
  ...
  if gamma == 0
     aux =  A'*Ilambda*A; % acho que isso não é usado em lugar nenhum
  ...
  invlambda = inv(diagonal + Wscovmatrix); % qual a diferença dessa p/ Ilambda?
  SVE = A*inv(A'*invlambda*A)*A';
  ASVEWLS = trace(SVE)/N;
