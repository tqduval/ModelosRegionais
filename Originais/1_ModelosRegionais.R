...
for(kmodel in 1:nrow(models)){ # kmodel" varia de 1 a 512
  ...
  # for(i in 1:n) wls.cov.matrix[i,i] <- 20
  
  # Confere se gamma = 0 é suficiente (pode ser que o otimizador retorne < 0)
  gamma <- 0                                                   # chute inicial
  wls.result <- fun.gls.var.delta(delta.in = gamma,            # resultado c/ chute inicial
                                  n = n,
                                  scovmatrix = wls.cov.matrix,
                                  theta = skew,
                                  X = X,
                                  p = p)
  
  check.gamma <- wls.result$check.delta
  check.gamma0 <- check.gamma
  gamma <- wls.result$delta
  par.wls <- wls.result$b
  lambda <- wls.result$lambda
  inv.lambda <- wls.result$inv.lambda
  ...
  if(gamma < 0){
    ...
    gamma <- wls.result.root$root                              # extrair 
    
    # Recalcular no novo delta
    wls.res.opt <- fun.gls.var.delta(delta.in = gamma,
                                     n = n,
                                     scovmatrix = wls.cov.matrix,
                                     theta = skew,
                                     X = X,
